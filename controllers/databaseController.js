  ...
  GameList.model.findById(req.params.id, function (err, gamelist) {
    ...
    Steam.getOwnedGames(gamelist.creatorSteamId, true, (err, allOwnedGames) => {
      ...
      gamelist.gameIds.forEach(function (gameId, index) {
        let foundUserInfo = allOwnedGames.find((game) => game.appid === gameId);
              ...
              (result) => {
                ...
                if (gameInfo[id].success) {
                  ...
                  finalInfo = {
                    ...
                    price: !gameInfo[id].data.is_free
                      ? gameInfo[id].data.price_overview.final_formatted
                      : undefined,
                    discount: !gameInfo[id].data.is_free
                      ? gameInfo[id].data.price_overview.discount_percent
                      : undefined,
                    genre: genres,
                    creatorPlaytime:
                      Math.round(
                        gamelistCreatorInfos.find((el) => el.appid === id)
